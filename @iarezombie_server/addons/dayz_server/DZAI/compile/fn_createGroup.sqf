...
for "_i" from 1 to _totalAI do {
	private ["_type","_unit"];
	diag_log format["FN_CREATEGROUP _groupSide = %1",_groupSide];
	if (_groupside == EAST) THEN {
		_type = DZAI_BanditTypes call BIS_fnc_selectRandom2;		// Select skin of AI unit
		diag_log format["FN_CREATEGROUP Side EAST, Type = %1",_type];

		};
		if (_groupside == WEST) THEN {
		_type = DZAI_WestTypes call BIS_fnc_selectRandom2;								// Select skin of AI unit
					diag_log format["FN_CREATEGROUP Side west, Type = %1",_type];

		};
		if (_groupside == RESISTANCE) THEN {
		_type = DZAI_ResistanceTypes call BIS_fnc_selectRandom2;								// Select skin of AI unit
					diag_log format["FN_CREATEGROUP Side resistance, Type = %1",_type];
